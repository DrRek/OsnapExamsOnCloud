...
export const make_api_call = async (
  ...
  data = null,
  other_params = "",
  tokenType = tokenRequest,
  return_all_response = false
) => {
...
export const check_resource_group_existance = async (name) => {
  const resp = await make_api_call(
    `resourcegroups/${name}`,
    "2021-04-01",
    "HEAD",
    null,
...
export const delete_all_resource_groups = async (prefix) => {
  if (!prefix) console.error("A prefix has to be specified");
  ...
  return await Promise.all(
    all_resources
      ...
      .map(({ name }) => delete_resource_group(name))
  );
};
...
export const create_subnet = async (resourceGroupName) =>
  make_api_call(
    `resourceGroups/${resourceGroupName}/providers/Microsoft.Network/virtualNetworks/customVirtualNetwork/subnets/customSubnet`,
    "2022-07-01",
    "PUT",
    {
      properties: {
...
export const create_public_ip_address = async (
  resourceGroupName,
  location = "westeurope"
) =>
  make_api_call(
    `resourceGroups/${resourceGroupName}/providers/Microsoft.Network/publicIPAddresses/customPublicIP`,
    "2022-07-01",
    "PUT",
    {
      location: location,
...
export const wait_for_ip_address = async (resourceGroupName) => {
  while (true) {
    ...
    if (result.properties.ipAddress) return result;
    await sleep(1000);
  }
};

export const create_network_security_group = async (
  resourceGroupName,
  location = "westeurope"
) =>
  make_api_call(
    `resourceGroups/${resourceGroupName}/providers/Microsoft.Network/networkSecurityGroups/customSecurityGroup`,
    "2022-07-01",
    "PUT",
    {
      location: location,
...
export const create_network_interface = async (
  name,
  ...
  make_api_call(
    `resourceGroups/${resourceGroupName}/providers/Microsoft.Network/networkInterfaces/${name}`,
    "2022-07-01",
    "PUT",
    {
      location: location,
...
export const create_network_interface_2 = async (
  name,
  subnetId,
  resourceGroupName = name,
  location = "westeurope"
...
export const create_virtual_machine = async (
  resourceGroupName,
  netInt1,
...
export const turn_on_virtual_machine = async (
  resourceGroupName,
  location = "westeurope"
) =>
  make_api_call(
    `resourceGroups/${resourceGroupName}/providers/Microsoft.Compute/virtualMachines/${VMNAME}/start`,
    "2022-11-01",
    "POST",
    {}
  );

export const turn_off_virtual_machine = async (
  resourceGroupName,
  location = "westeurope"
) =>
  make_api_call(
    `resourceGroups/${resourceGroupName}/providers/Microsoft.Compute/virtualMachines/${VMNAME}/deallocate`,
    "2022-11-01",
    "POST",
    {}
  );

export const check_virtual_machine = async (
  resourceGroupName,
  location = "westeurope"
) =>
  make_api_call(
    `/resourceGroups/${resourceGroupName}/providers/Microsoft.Compute/virtualMachines/${VMNAME}/instanceView`,
    "2022-11-01",
    "GET"
  );

export const create_budget_alert = async (
  resourceGroupName,
  location = "westeurope"
) =>
  make_api_call(
    `resourceGroups/${resourceGroupName}/providers/Microsoft.Consumption/budgets/budget`,
    "2021-10-01",
    "PUT",
    {
      ...
      properties: {
        ...
        notifications: {
          Actual_GreaterThan_80_Percent: {
            ...
            threshold: 80,
            locale: "it-it",
            contactEmails: ["lucareccia@hotmail.it"],
...
export const change_vm_passwords = async (
  resourceGroupName,
  adminPw,
  studentPw,
  storageContainerName = "testcontainername",
  storageContainerToken = "expiredkeytoreplace",
  downloadDesktopUrl = null
) => {
  const changePwCmds = [
  ...
  const setBackupScriptCmds = [
    ...
    `cmd.exe /C 'C:\\Windows\\System32\\chcp.com 65001 & echo C:\\Users\\studente\\azcopy_windows_amd64_10.20.1\\azcopy.exe sync "C:\\Users\\studente\\Desktop" "https://osnapdbexamsonthecloud.blob.core.windows.net/${storageContainerName}?${storageContainerToken.replaceAll(
      "%",
      "%%"
    )}" --delete-destination true >> C:\\Users\\studente\\azcopy_windows_amd64_10.20.1\\syncer.bat'`,
  ...
  const resp = await make_api_call(
    `resourceGroups/${resourceGroupName}/providers/Microsoft.Compute/virtualMachines/customVirtualMachine/runCommand`,
    "2019-03-01",
    "POST",
    {
...
export const check_create_user_in_vm = async (exam) => {
  try {
    ...
    if (!location_header) return false;
...
export const create_storage_container = async (name) => {
  const resp = await make_api_call(
    `resourceGroups/Managment-ExamsOnTheCloud/providers/Microsoft.Storage/storageAccounts/osnapdbexamsonthecloud/blobServices/default/containers/${name}`,
    "2023-01-01",
    "PUT",
    {
      properties: {
...
export const create_storage_container_sas = async (name) =>
  make_api_call(
    `resourceGroups/Managment-ExamsOnTheCloud/providers/Microsoft.Storage/storageAccounts/osnapdbexamsonthecloud/listServiceSas`,
    "2023-01-01",
    "POST",
    {
...
export const db_list_active_exams_v2 = async () => {
  ...
  const resp = await fetch(
    `${DB_URL}()?$filter=status ne '${E_STATUS_VALUES.DESTROYED}'`,
    options
  );
  return (await resp.json()).value.map((i) => db_util_json_to_obj(i));
};
...
export const db_is_prefix_unique_v2 = async (newID) => {
  ...
  const resp = await fetch(
    `${DB_URL}()?$filter=RowKey ge '${newID}' and RowKey lt '${newID}z'`,
    options
  );
...
export const db_update_exam_v2 = async (exam, reason) => {
  ...
  const resp = await fetch(
    `${DB_URL}(PartitionKey='Osnap',RowKey='${exam.id}')`,
    options
  );
};
